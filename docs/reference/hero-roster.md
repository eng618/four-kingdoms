# Reference: Hero Roster (v1 — 12 sets)

Placeholders until real names land. Replace `element_0X` / `forgotten_01` codenames in place; IDs below are the intended registry names.

| Kingdom   | Hero (placeholder)     | Registry prefix | Repair ingredient (proposal) | Signature weapon   |
| --------- | ---------------------- | --------------- | ---------------------------- | ------------------ |
...
| Element   | Flare                  | flare           | blaze_rod + iron             | Sword              |
...
* One row = one `ArmorMaterial` + one `Tier` + armor(4) + weapon(1) + tools(up to 4).
* Element heroes should map 1:1 to sub-elements (fire/storm/earth/ocean/…) — fill the TBD columns when lore lands.
* Wild Kingdom stays an empty tab placeholder until a hero is defined; do not register dead items.
